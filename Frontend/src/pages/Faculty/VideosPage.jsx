...
const UploadedVideosPage = () => {
  ...
  return (
    <div className={`p-8 min-h-screen transition-colors duration-300 ${isDarkMode ? "bg-gray-900" : "bg-white"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
        <select value={program} onChange={(e) => setProgram(e.target.value)} className={selectClass(isDarkMode)}>
          <option value="">All Programs</option>
          {uniqueOptions("program").map((prog, idx) => (
            <option key={idx}>{prog}</option>
          ))}
        </select>
        <select value={branch} onChange={(e) => setBranch(e.target.value)} className={selectClass(isDarkMode)}>
          <option value="">All Branches</option>
          {uniqueOptions("branch").map((br, idx) => (
            <option key={idx}>{br}</option>
          ))}
        </select>
        <select value={semester} onChange={(e) => setSemester(e.target.value)} className={selectClass(isDarkMode)}>
          <option value="">All Semesters</option>
          {uniqueOptions("semester").map((sem, idx) => (
            <option key={idx}>{sem}</option>
          ))}
        </select>
        <select value={subject} onChange={(e) => setSubject(e.target.value)} className={selectClass(isDarkMode)}>
      ...
      {loading ? (
        <p className="text-center animate-pulse text-lg">Loading your videos...</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {videos.map((video) => (
            <div key={video._id} className={`overflow-hidden border rounded-lg shadow-md ${isDarkMode ? "bg-gray-800" : "bg-white"}`}>
              <div
              ...
              <div className="p-4 flex justify-between items-start">
                <div>
                  <h3 className={`text-sm font-bold truncate ${isDarkMode ? "text-gray-100" : "text-gray-800"}`}>{video.title}</h3>
                  ...
                  <div className={`text-xs mt-2 ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
                    ...
                    <span>{video.uploadedAtFormatted || new Date(video.createdAt).toLocaleDateString()}</span>
                  </div>
                </div>
                <div className="py-4">
                  <button
                    ...
                    className={`ml-4 h-fit px-3 py-1 text-sm font-semibold rounded-lg ${
                      isDarkMode ? "bg-gray-700 text-gray-100 hover:bg-gray-600" : "bg-red-500 text-white hover:bg-red-600"
                    }`}
                  >
                    Delete
                  </button>
